...
st.set_page_config(page_title="Tri — Graphix", page_icon="📊", layout="wide")
inject_css()
sidebar_nav()
...
def bubble_sort_steps(arr):
    ...
    for i in range(n):
        for j in range(0, n - i - 1):
            ...
            descriptions.append(
                f"Comparaison : a[{j}]={a[j]} et a[{j+1}]={a[j+1]}"
            )
            if a[j] > a[j + 1]:
                a[j], a[j + 1] = a[j + 1], a[j]
                steps.append((a.copy(), j, j + 1, list(range(n - i, n))))
                descriptions.append(
                    f"Échange : a[{j}] ↔ a[{j+1}] → {a[j+1]} avant {a[j]}"
                )
...
def merge_sort_steps(arr):
    ...
    def merge_sort(arr, left):
        ...
        while i < len(L) and j < len(R):
            ...
            descriptions.append(
                f"Fusion : comparaison L[{i}]={L[i]} vs R[{j}]={R[j]}"
            )
            if L[i] <= R[j]:
                merged.append(L[i])
                i += 1
        ...
        descriptions.append(
            f"Sous-tableau [{left}:{left+len(merged)}] fusionné"
        )
        return merged
...
def quick_sort_steps(arr):
    ...
    def quick_sort(lo, hi):
        ...
        for j in range(lo, hi):
            ...
            descriptions.append(
                f"Comparaison : a[{j}]={a[j]} vs pivot={pivot}"
            )
            if a[j] <= pivot:
                a[i], a[j] = a[j], a[i]
...
# ── UI ────────────────────────────────────────────────────────────────────────
st.markdown(
    '<span class="page-badge" style="background:rgba(124,58,237,0.15);border:1px solid rgba(124,58,237,0.3);color:#a78bfa;">📊 ALGORITHMES DE TRI</span>',
    unsafe_allow_html=True,
)
st.markdown(
    '<div class="page-title">Tri & Comparaisons</div>', unsafe_allow_html=True
)
st.markdown(
    '<div class="page-desc">Observez chaque étape de 3 algorithmes de tri. Utilisez le bouton <b>▶ Démarrer</b> ou le slider pour naviguer. Les barres jaunes = comparaison, rouge = pivot, vert = trié.</div>',
    unsafe_allow_html=True,
)
...
with col_ctrl:
    ...
    algo = st.selectbox(
        "Algorithme", ["Tri à Bulles", "Tri Fusion", "Tri Rapide"]
    )
    ...
    st.markdown(
        f'<span class="complexity-badge">{complexity}</span>',
        unsafe_allow_html=True,
    )
    st.markdown(
        f'<span class="complexity-badge" style="margin-top:6px;display:inline-block;">{len(steps)} étapes</span>',
        unsafe_allow_html=True,
    )
