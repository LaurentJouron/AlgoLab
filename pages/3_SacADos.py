...
st.set_page_config(page_title="Sac à Dos — Graphix", page_icon="🎒", layout="wide")
inject_css()
sidebar_nav()
    ...
    for i in range(1, n+1):
        for w in range(capacity+1):
            if weights[i-1] <= w:
                ...
                action = (f"Objet {i} ({values[i-1]}€, {weights[i-1]}kg) → PRIS (gain {take} > {notake})"
                          if take > notake else
                          f"Objet {i} ({values[i-1]}€, {weights[i-1]}kg) → IGNORÉ ({notake} ≥ {take})")
            else:
                dp[i][w] = dp[i-1][w]
...
def make_items_bar(weights, values, n, chosen_indices, current_item):
    colors = ["#10b981" if i in chosen_indices else "#f59e0b" if i == current_item else "#334155" for i in range(n)]
    return [
        go.Bar(name="Valeur (€)", x=[f"Obj {i+1}" for i in range(n)], y=values,
               marker_color=colors, text=[f"{v}€" for v in values],
               textposition='auto', textfont=dict(color='white', size=10)),
        go.Bar(name="Poids (kg)",  x=[f"Obj {i+1}" for i in range(n)], y=weights,
...
def make_animated_fig(weights, values, names, capacity, steps, chosen):
    ...
    fig = make_subplots(
        ...
        subplot_titles=["Objets — Valeur & Poids", "Table de programmation dynamique"],
    ...
    fig.add_trace(make_dp_heatmap(dp0[:si0+1, :capacity+1], si0, sw0), row=2, col=1)
    ...
    fig.update_layout(
        ...
        height=580,
        annotations=[
            dict(text="Objets — Valeur & Poids", x=0.5, y=1.04,
                 xref='paper', yref='paper', showarrow=False,
                 font=dict(color='#94a3b8', size=12)),
            dict(text="Table de programmation dynamique", x=0.5, y=0.6,
                 xref='paper', yref='paper', showarrow=False,
                 font=dict(color='#94a3b8', size=12)),
            dict(text=act0, x=0.5, y=1.09, xref='paper', yref='paper',
                 showarrow=False, font=dict(color='#fcd34d', size=11, family='DM Sans'),
                 align='center'),
        ],
        # Cellule active (shape)
        shapes=[dict(type="rect",
            x0=sw0-0.5, x1=sw0+0.5, y0=si0-0.5, y1=si0+0.5,
        ...
        sliders=[dict(
            active=0,
            currentvalue=dict(prefix="Cellule : ",
            ...
            steps=[dict(method="animate",
                        args=[[f"k{k}"], dict(mode="immediate",
            ...
            layout=go.Layout(
                annotations=[
                    ...
                    dict(text="Table de programmation dynamique", x=0.5, y=0.6,
                         xref='paper', yref='paper', showarrow=False,
                         font=dict(color='#94a3b8', size=12)),
                    dict(text=action, x=0.5, y=1.09, xref='paper', yref='paper',
                         showarrow=False,
                         font=dict(color='#fcd34d', size=11, family='DM Sans'),
                         align='center'),
                ],
                shapes=[dict(type="rect",
                    x0=sw-0.5, x1=sw+0.5, y0=si-0.5, y1=si+0.5,
                    line=dict(color="#f59e0b", width=3),
                    fillcolor="rgba(245,158,11,0.15)",
                    xref='x2', yref='y2',
                )],
...
# ── UI ────────────────────────────────────────────────────────────────────────
st.markdown('<span class="page-badge" style="background:rgba(245,158,11,0.15);border:1px solid rgba(245,158,11,0.3);color:#fcd34d;">🎒 SAC À DOS</span>', unsafe_allow_html=True)
st.markdown('<div class="page-title">Sac à Dos 0/1</div>', unsafe_allow_html=True)
st.markdown('<div class="page-desc">Programmation dynamique : la cellule jaune montre la décision en cours. Utilisez <b>▶ Démarrer</b> ou le slider pour avancer.</div>', unsafe_allow_html=True)
...
with col_ctrl:
    ...
    for i in range(n_items):
        d = DEFAULT_ITEMS[i] if i < len(DEFAULT_ITEMS) else {"nom": f"Objet {i+1}", "poids": i+1, "valeur": i+2}
        with st.expander(f"Objet {i+1} — {d['nom']}"):
            p = st.number_input("Poids (kg)", min_value=1, max_value=capacity, value=min(d['poids'], capacity), key=f"p{i}")
            v = st.number_input("Valeur (€)", min_value=1, max_value=20,       value=d['valeur'],               key=f"v{i}")
...
with col_viz:
    ...
    st.plotly_chart(fig, width='stretch', key=f"ks_{n_items}_{capacity}_{weights}")
