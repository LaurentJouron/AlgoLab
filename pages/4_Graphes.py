...
st.set_page_config(
    page_title="Graphes — Graphix", page_icon="🕸️", layout="wide"
)
inject_css()
sidebar_nav()
...
def build_frame_traces(nodes, edges, pos, visited, current, path_edges, dist):
    ...
    for u, v, w in edges:
        ...
        traces.append(
            go.Scatter(
                ...
                mode="lines",
                line=dict(
                    color="#06b6d4" if is_path else "#1e2e3e",
    ...
    node_colors = [
        "#f59e0b" if n == current else "#10b981" if n in visited else "#334155"
        for n in nodes
    ]
    node_sizes = [
        30 if n == current else 24 if n in visited else 22 for n in nodes
    ]
    traces.append(
        go.Scatter(
            ...
            hovertext=[
                (
                    f"{n}: {int(dist[n])} km"
                    if dist and dist.get(n, float("inf")) != float("inf")
                    else n
                )
                for n in nodes
            ],
...
def make_animated_fig(
    nodes, edges, pos, steps, algo, end_node, dist_final, prev_final
):
    n_steps = len(steps)
...
# ── UI ────────────────────────────────────────────────────────────────────────
st.markdown(
    '<span class="page-badge" style="background:rgba(16,185,129,0.15);border:1px solid rgba(16,185,129,0.3);color:#6ee7b7;">🕸️ ALGORITHMES DE GRAPHES</span>',
    unsafe_allow_html=True,
)
st.markdown(
    '<div class="page-title">Graphes & Parcours</div>', unsafe_allow_html=True
)
st.markdown(
    '<div class="page-desc">Visualisez Dijkstra, BFS et DFS. Utilisez <b>▶ Démarrer</b> ou le slider pour avancer. Nœuds verts = visités, jaune = en cours, bleu = chemin optimal.</div>',
    unsafe_allow_html=True,
)
...
with col_ctrl:
    ...
    st.markdown(
        f'<span class="complexity-badge">{complexity_map[algo]}</span>',
        unsafe_allow_html=True,
    )
    st.markdown("---")
    ...
    if algo == "Dijkstra":
        end_node = st.selectbox(
            "Nœud d'arrivée", [n for n in g["nodes"] if n != start_node]
        )
    ...
    st.markdown(
        f'<div class="info-box" style="border-left-color:#10b981;font-size:0.82rem;">{desc_map[algo]}</div>',
        unsafe_allow_html=True,
    )
...
if algo == "Dijkstra":
    steps, dist_final, prev_final = dijkstra_steps(
        g["nodes"], g["edges"], start_node
    )
elif algo == "BFS":
...
with col_viz:
    ...
    st.plotly_chart(
        fig, width="stretch", key=f"gr_{graph_name}_{algo}_{start_node}"
    )
    ...
    if algo == "Dijkstra" and dist_final:
        ...
        for i, node in enumerate(g["nodes"]):
            ...
            color = (
                "#f59e0b"
                if node == start_node
                else ("#10b981" if d != float("inf") else "#ef4444")
            )
            cols[i].markdown(
                f"""
